 ...
 * Traverses into first shadow root to find and hide the bubble button only.
 * Does NOT hide the chat panel itself - only the floating launcher button.
 */
export function hideLauncher(): void {
  ...
  if (shadowRoot) {
    // Only target the launcher bubble button - NOT the entire widget
