...
| 6.16 | Decouple gold accent from theme system | `in-progress` | B1 | Gold persists on checkout; homepage logo stays purple | Dedicated `--gold` CSS var immune to theme overrides. **DEFERRED TO LAST** – Site colors are severely broken; requires manual design intervention. Lovable AI is not reliable for visual design decisions. Will be addressed as a comprehensive color/design pass after all functional tasks are complete. |
...
| 6.24 | **Sales Infrastructure (Matrix v1.2)** | `todo` | B6 | Abandoned cart, affiliate tracking, chat pipeline integration | Expanded into sub-tasks 6.24.1–6.24.7 below. Authority: `docs/e-commerce-feature-matrix-v1.2.md` |
| 6.24.1 | GHL prep: verify tags, pipeline, Affiliate ID custom field | `done` | B6 | All 29 `EI:` tags confirmed; 5-stage Checkout Pipeline confirmed; `EI: Chat – Inquiry` present | **Phase 0.** Verified 2026-02-10 via ChatGPT GHL audit. Remaining: verify GHL custom field `Affiliate ID` exists for passthrough. |
...
| 6.24.5 | All 3 edge functions: affiliate `affiliate_id` passthrough + `EI: Affiliate – [ID]` tag | `todo` | B6 | `start-checkout`, `submit-form`, `submit-job-application` accept `affiliate_id` in POST body → save to DB column → apply GHL tag | **Phase 3b.** Depends on 6.24.2 (DB columns) + 6.24.3 (cookie reader). Matrix features 2.2, 2.3. ~3.5 hours. |
...
| 6.24.7 | End-to-end validation: 3 scenarios (abandon/recover, affiliate, chat→pipeline) | `todo` | B7 | SC1: anon→checkout→abandon→recover→pay. SC2: `?ref=PARTNER123`→form→affiliate tag+commission. SC3: chat→`EI: Chat – Inquiry`→Pre-Checkout→checkout→affiliate preserved | **Phase 5.** Matrix Section 7 test scenarios. Run on everintent.com production. |
