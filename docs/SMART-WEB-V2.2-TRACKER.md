...
| 8.11 | Shared admin UI patterns: `AdminListView<T>`, `AdminDetailView`, `AdminFormEditor<T>` | `done` | 8.10 | `src/modules/shared/types.ts` (FieldDef + ColumnDef contracts), `AdminListView.tsx` (data table + skeleton + empty state + create button), `AdminDetailView.tsx` (back nav + title + action bar + content slot), `AdminFormEditor.tsx` (react-hook-form + Zod resolver + 7 field types: text, textarea w/ char counter, number, boolean/Switch, select, tags, image-url w/ preview). All fully JSDoc'd. |
| 8.12 | Portfolio admin CRUD (replace Placeholder) | `done` | 8.11 | `src/modules/portfolio/service.ts` (Zod schemas + CrudService + hooks), `PortfolioListPage.tsx` (AdminListView with 5 columns), `PortfolioEditPage.tsx` (AdminFormEditor with 13 fields, create/edit/delete). Module registration updated to v2.0.0 with real routes including `:id` param. |
| 8.13 | Testimonials admin CRUD (replace Placeholder) | `done` | 8.11 | `src/modules/testimonials/service.ts` (Zod schemas + CrudService + hooks), `TestimonialsListPage.tsx` (AdminListView with 5 columns incl. star rating), `TestimonialsEditPage.tsx` (AdminFormEditor with 11 fields, create/edit/delete). Module registration updated to v2.0.0. |
| 8.14 | Module permission system (`requiredRole` enforcement) | `done` | 8.7 | `src/hooks/useHasRole.ts` (TanStack Query cached RPC check). Dashboard.tsx updated with `canAccess()` filter on navItems. Supports admin/moderator/user role hierarchy. |
| 8.15 | Full QA: verify all 5 modules render, navigate, and save correctly | `todo` | 8.12, 8.13 | Cross-module test: dashboard cards → drill-down → CRUD operations. Also: Theme Detail View enhanced with Motion, Style Modules detail, Logo, ADA detail sections. |
...
| 2026-02-12 | **Task 8.11 COMPLETE**: Shared admin UI patterns. Created `src/modules/shared/types.ts` (`FieldDef` + `ColumnDef<T>` contracts), `AdminListView.tsx` (data table + skeleton + empty state + create button), `AdminDetailView.tsx` (back nav + title + subtitle + action bar + content slot + loading skeleton), `AdminFormEditor.tsx` (react-hook-form + Zod resolver + 7 field types: text, textarea w/ char counter, number, boolean/Switch, select, tags, image-url w/ preview). Barrel re-exports all. Full JSDoc on every file. | Lovable |
| 2026-02-12 | **Tasks 8.12–8.14 COMPLETE**: (8.12) Portfolio CRUD — `service.ts` (Zod schemas, CrudService, hooks), `PortfolioListPage.tsx`, `PortfolioEditPage.tsx` (13 fields). Module v2.0.0. (8.13) Testimonials CRUD — `service.ts`, `TestimonialsListPage.tsx`, `TestimonialsEditPage.tsx` (11 fields). Module v2.0.0. (8.14) Permission enforcement — `useHasRole.ts` hook (cached RPC), Dashboard `canAccess()` filter. Theme Detail View enhanced with Motion, Style Modules detail, Logo, ADA sections. | Lovable |
| 2026-02-12 | **Theme Hub premium redesign**: ThemeListView rebuilt as visual grid (1/2/3 cols) with gradient hero banners, search, framer-motion animations, palette previews. ThemeDetailView rebuilt as self-contained dashboard with full-width hero, 9 Section cards (palettes, typography with live preview, CTA rendered buttons, motion, modules, integrations, logo). | Lovable |
| 2026-02-13 | **Shopify + Shadcn hybrid migration**: Eliminated ThemeDetailView (3-level → 2-level: Hub → Editor). ThemeListView rebuilt with ThemeMockup cards rendering actual theme tokens (nav, hero, CTA, cards, gold accent bar). ThemeEditorView rebuilt as split-screen: ~360px control sidebar (left) with ThemeEditorNav + ThemeEditorPanels, ThemeLiveCanvas (right) rendering full website preview (nav, hero, trust bar, service cards, testimonial, CTA form, footer). Mobile: Controls/Preview segmented toggle. `useThemeAdmin.ts`: `ThemeAdminView = 'list' \| 'editor'`, `selectTheme` → editor directly. New file: `ThemeLiveCanvas.tsx`. JSDoc updated in Themes.tsx and useThemeAdmin.ts. | Lovable |
...
## Phase 10 – Core Web Vitals & PageSpeed Optimization (BRD v36.7) 🚧 IN PROGRESS
...
| ID | Task | Status | Verification | Notes |
|----|------|--------|--------------|-------|
| 10.1 | Non-render-blocking font loading | `done` | Google Fonts uses `rel="preload"` with async onload in `index.html` | BRD v36.7 §D1. Saves ~750ms render-blocking time |
...
| 10.5 | JS bundle code splitting | `in-progress` | P0 repo cleanup (v36.8). P2 admin lazy loading complete (v36.11): 4 admin imports + 6 module page imports converted to React.lazy(). Suspense added to AdminLayout. Public bundle no longer includes admin code. P3 (visualizer) and P4 (lucide audit) still pending. | BRD v36.11 §D5. P2 done. Run PageSpeed to measure. |
