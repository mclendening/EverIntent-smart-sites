...
| 6.16 | Decouple gold accent from theme system | `in-progress` | (1) Refresh checkout page – gold pricing must persist, no flash to purple; (2) Verify logo on homepage still shows purple 'Intent' text; (3) Confirm `text-gold`, `bg-gold`, `border-gold` classes render correctly across all checkout steps | **Root Cause:** `applyThemeToRoot()` in `themes.ts` overrides `--accent` at runtime, causing gold to flash then disappear. **Solution:** Created dedicated `--gold` CSS variable (HSL `39 95% 50%`) that is immune to theme overrides. **Design Principle:** `--accent` (purple) = brand/logo identity; `--gold` = pricing/checkout luxury accent — must remain permanently separate. **Files Modified:** `src/index.css` (added `--gold` variable), `tailwind.config.ts` (added `gold` color utility), `src/components/checkout/CheckoutStep1Selection.tsx` (switched `text-accent` → `text-gold`), `src/components/checkout/CheckoutStep3Review.tsx` (switched `text-accent` → `text-gold`), `src/components/checkout/OrderSummary.tsx` (switched `text-accent` → `text-gold`). **Tailwind Classes Available:** `text-gold`, `bg-gold`, `border-gold`. **Pending:** User verification on production refresh, logo color check, memory update confirmation. |
| 6.17 | Full-site CTA & UX flow audit – route all product CTAs to checkout | `todo` | Every "Get Started", "Choose Plan", "Add to Plan", and primary CTA across the entire site routes to the correct `/checkout/[tier]` page. No circular flows back to detail pages. No orphan CTAs pointing to `/contact` for purchase intent. | **Scope:** Step-by-step expert UX review of every page that has a purchase-intent CTA. **Problem:** Current CTAs were built to funnel to `/contact` or loop back to product detail pages, creating circular dead-end flows with no conversion path to checkout. **Pages to audit:** (1) Smart Website tier pages: `/smart-websites/smart-site`, `/smart-websites/smart-lead`, `/smart-websites/smart-business`, `/smart-websites/smart-growth` – all "Get Started" CTAs; (2) AI Employee tier pages: `/let-ai-handle-it/after-hours`, `/let-ai-handle-it/front-office`, `/let-ai-handle-it/full-ai-employee` – all "Get Started" CTAs; (3) `/compare-websites` – all "Choose [Tier]" buttons; (4) `/compare-ai-employee` – all "Choose [Plan]" buttons; (5) `/pricing` – all tier card CTAs on both tabs; (6) `/smart-websites/add-ons` hub – all "Add to Plan" buttons (currently → `/contact`); (7) Recommended Add-Ons sections on each tier page; (8) `/industries/*` showcase pages – all product recommendation CTAs; (9) Homepage hero, pricing teaser, and final CTA sections; (10) Scale tier After-Hours upsell link. **Deliverable:** For each page, document current CTA target vs. correct target, then update. **Supersedes:** Tasks 6.12 and 6.13 are subsumed by this task. **Dependency:** Checkout steps 1-3 must be functional first (tasks 6.4, 6.5). |
