...
| 7.24 | Implement user-facing light/dark mode toggle (header + mobile + `<head>` script) | `done` | 7.7 | `ModeToggle.tsx` component added to desktop header (compact pill) and mobile menu (expanded). FOUC script in `index.html` reads `localStorage('theme-mode')`. Light mode CSS variables added to `index.css`. |
